...
const gridRef = useRef<THREE.Mesh>(null);
  ...
  // Stable across re-renders so Model's onLoad effect only fires once per
  // actual model load, not on every selection/color-override change.
  const handleLoad = useCallback(
    (box: THREE.Box3) => {
      ...
      if (controlsRef.current) {
        ...
        // Fallback if controls aren't mounted yet for some reason.
